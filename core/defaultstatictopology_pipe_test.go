		...
		for par := 1; par <= maxPar; par++ {
			...
			Convey(fmt.Sprintf("When tuples are emitted with parallelism %d", par), func() {
				...
				So(waitUntil(func() bool {
					return len(si.Tuples) >= len(so.Tuples)
				}), ShouldBeFalse)
	...
	Convey("Given a simple source/box/sink topology with 2 boxes and 2 sinks", t, func() {
		...
		for par := 1; par <= maxPar; par++ {
			...
			Convey(fmt.Sprintf("When tuples are emitted with parallelism %d", par), func() {
				...
				// wait until tuples arrived
				So(waitUntil(func() bool {
					return len(si1.Tuples) >= len(so.Tuples) && len(si2.Tuples) >= len(so.Tuples)
				}), ShouldBeFalse)
	...
	Convey("Given a simple source/box/sink topology with 2 sources", t, func() {
		...
		for par := 1; par <= maxPar; par++ {
			...
			Convey(fmt.Sprintf("When tuples are emitted with parallelism %d", par), func() {
				...
				// wait until tuples arrived
				So(waitUntil(func() bool {
					return len(si.Tuples) >= len(so1.Tuples)+len(so2.Tuples)
				}), ShouldBeFalse)
...
// wait until the function given as parameter becomes true
func waitUntil(condition func() bool) (timedOut bool) {
	loopTimeout := 30
	i := 0
