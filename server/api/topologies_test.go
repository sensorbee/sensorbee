...
func TestEmptyTopologies(t *testing.T) {
	...
	Convey("Given an API server", t, func() {
		Convey("When creating a topology", func() {
			res, js, err := cli.do(cmd.PostRequest, "/topologies", map[string]interface{}{
				"name": "test_topology",
			})
			So(err, ShouldBeNil)
			So(res.StatusCode, ShouldEqual, http.StatusOK)
			Reset(func() {
				cli.do(cmd.DeleteRequest, "/topologies/test_topology", nil)
			})
			...
			Convey("Then getting the topology should succeed", func() {
				res, js, err := cli.do(cmd.GetRequest, "/topologies/test_topology", nil)
				So(err, ShouldBeNil)
				So(res.StatusCode, ShouldEqual, http.StatusOK)
				So(jscan(js, "/topology/name"), ShouldEqual, "test_topology")
			})
			...
			Convey("And getting a list of topologies", func() {
				res, js, err := cli.do(cmd.GetRequest, "/topologies", nil)
				So(err, ShouldBeNil)
				So(res.StatusCode, ShouldEqual, http.StatusOK)
		...
		Convey("When deleting a nonexistent topology", func() {
			res, _, err := cli.do(cmd.DeleteRequest, "/topologies/test_topology", nil)
			So(err, ShouldBeNil)
...
func TestTopologiesCreateInvalidValues(t *testing.T) {
	...
	Convey("Given an API server", t, func() {
		...
		Convey("When posting a broken JSON request", func() {
			res, js, err := cli.do(cmd.PostRequest, "/topologies", json.RawMessage("{broken}"))
			So(err, ShouldBeNil)
