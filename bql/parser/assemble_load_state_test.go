...
func TestAssembleLoadState(t *testing.T) {
	...
	Convey("Given a parser", t, func() {
		...
		Convey("When doing a full LOAD STATE included inner map", func() {
			p.Buffer = `LOAD STATE a_1 TYPE b TAG t SET c=[{"d":"e"},["f","g"]]`
			p.Init()
			...
			Convey("Then the statement should be parsed correctly", func() {
				...
				So(comp.Params[0].Key, ShouldEqual, "c")
				actualSetValue, err := data.AsArray(comp.Params[0].Value)
				So(err, ShouldBeNil)
				So(len(actualSetValue), ShouldEqual, 2)

				expectedValue0 := data.Map{
					"d": data.String("e"),
				}
				So(actualSetValue[0], ShouldResemble, expectedValue0)
				expectedValue1 := data.Array{data.String("f"), data.String("g")}
				So(actualSetValue[1], ShouldResemble, expectedValue1)
