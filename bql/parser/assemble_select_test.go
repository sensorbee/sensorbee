...
func TestAssembleSelect(t *testing.T) {
	Convey("Given a parseStack", t, func() {
		...
		Convey("When the stack contains the correct SELECT items", func() {
			...
			ps.PushComponent(14, 15, Stream{"d"})
		...
		Convey("When the stack contains the correct SELECT items but no emitter", func() {
			ps.PushComponent(6, 7, RowValue{"", "a"})
			ps.PushComponent(7, 8, RowValue{"", "b"})
			ps.AssembleProjections(6, 8)
			ps.PushComponent(12, 13, Stream{"c"})
	...
	Convey("Given a parser", t, func() {
		...
		Convey("When doing a full SELECT without an emitter", func() {
