...
func createDummyUpdatableSource(ctx *core.Context, ioParams *IOParams, params data.Map) (core.Source, error) {
	numTuples := 4
	// check the given source parameters
	for key, value := range params {
		if key == "num" {
...
func createCollectorSink(ctx *core.Context, ioParams *IOParams, params data.Map) (core.Sink, error) {
